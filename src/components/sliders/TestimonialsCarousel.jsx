    ...
    <section className="my-10">
      ...
      <div className="px-5 sm:px-8 lg:px-15">
        <div className="flex flex-row-reverse items-center justify-start gap-2 mb-4 sm:mb-5">
          <button onClick={() => emblaApi?.scrollPrev()} aria-label="Previous" className={btnCls}>
            →
          </button>
          <button onClick={() => emblaApi?.scrollNext()} aria-label="Next" className={btnCls}>
            ←
          </button>
        </div>
        <div className="overflow-hidden cursor-grab active:cursor-grabbing" ref={emblaRef} dir="ltr">
          <div className="flex">
            {displayPairs.map((pair, i) => (
              <div key={i} className="shrink-0 w-4/5 sm:w-1/2 lg:w-1/4 pl-5 flex flex-col gap-5">
                {pair.map((item) => (
