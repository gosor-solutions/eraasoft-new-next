...
export default function NavLinks({
  ...
  const links = [
    ...
    {
      label: navbarTitles?.learning_journey || defaultTitles.learning_journey,
      href: "/journey",
    },
    {
      label: navbarTitles?.testimonials || defaultTitles.testimonials,
      href: "/reviews",
    },
    {
    ...
    {
      label: navbarTitles?.contact_us || defaultTitles.contact_us,
      href: "/contact",
    },
  ];
            ...
            {links?.map((link) => (
              <li key={link.href}>
                <Link
                  href={link.href}
                  onClick={() => setOpenModal(false)}
                  className={`flex items-center justify-start gap-3 px-4 py-3 rounded-lg text-[15px] font-medium transition-all hover:bg-blue-50 hover:text-(--primary-color) ${
                    pathName === link.href
