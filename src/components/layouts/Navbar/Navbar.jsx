    ...
    <>
      <header className="px-6 py-4 flex justify-between items-center bg-white shadow-sm sticky top-0 z-50">
        <div className="flex items-center gap-2">
          {contactPhone && (
            <a href={`https://wa.me/${contactPhone.replace(/\D/g, "")}`} target="_blank" rel="noopener noreferrer" className="hidden min-[868px]:flex items-center gap-2 main_button whats_app-button" aria-label="تواصل معنا على واتساب">
              <WhatsAppIcon />
              <span className="hidden sm:inline">واتساب</span>
            </a>
          )}
          <button className="md:hidden p-2 rounded-md border border-gray-200 hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => setOpenModal(!openModal)} aria-label="toggle menu">
