...
export const metadata = {
  ...
  description: "تعرف على سياسة الخصوصية وسياسة الاسترجاع الخاصة بـ EraaSoft — كيف نحمي بياناتك وما هي شروط استرجاع الرسوم الدراسية.",
};
...
const sections = [
  {
    ...
    intro: "في EraaSoft، نلتزم بالحفاظ على سرية وأمان بيانات مستخدمينا، سواء كانوا عملاء تطوير برمجيات أو متدربين في برامجنا التعليمية.",
    subsections: [
      {
      ...
      },
      {
        ...
        items: ["تحسين الخدمات التعليمية والتقنية لدينا.", "تقديم الدعم الفني والتواصل عند الحاجة.", "إرسال التحديثات والمحتوى المرتبط بخدماتنا، بموافقتك المسبقة."],
      },
    ],
  ...
  },
  {
    ...
    intro: "في EraaSoft، نُقدّر التزام المتدربين معنا ونسعى لتوفير تجربة مريحة وعادلة للجميع. لذلك، نسمح باسترجاع الرسوم الدراسية وفقًا للضوابط التالية:",
    subsections: [
      {
    ...
    <div className="bg-[#FAFAFA]" dir="rtl">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        {sections.map((section) => (
          <section key={section.id} className="mb-14">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-2xl sm:text-3xl font-bold text-[#0C1739]">{section.title}</h2>
            </div>

            <p className="text-gray-600 leading-relaxed mb-8 text-base sm:text-lg">{section.intro}</p>

            <div className="flex flex-col gap-6">
              {section.subsections.map((sub) => (
                <div key={sub.title} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                  <h3 className="text-lg font-bold text-[#2243A4] mb-4">{sub.title}</h3>
            ...
            {section.note && (
              <div className="mt-6 bg-[#EEF2FF] border border-[#2243A4]/20 rounded-2xl p-5">
                <p className="text-[#2243A4] font-semibold text-sm sm:text-base leading-relaxed">{section.note}</p>
              </div>
            )}
          </section>
        ))}
        ...
        <div className="bg-[#0C1739] rounded-2xl p-6 sm:p-8 text-center">
          <h3 className="text-[#BECBF2] text-lg font-bold mb-2">للتواصل معنا</h3>
          <p className="text-white text-sm sm:text-base mb-4">لأي استفسارات تتعلق بالخصوصية أو الاسترجاع، يرجى التواصل عبر البريد الإلكتروني:</p>
